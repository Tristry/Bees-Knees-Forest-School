...
{#if club}
  <section class="container pt-1">
    ...
    <div class="flex justify-between max-md:flex-col gap-10 mt-3">
      <p
        class="mt-4 leading-7 w-4/5 max-md:w-full max-md:text-sm max-md:leading-6"
      >
        {club?.description}
      </p>
      <form
        ...
        use:enhance={({ formElement, formData, action, cancel, submitter }) => {
          ...
          formData.set(
            "successUrl",
            $page.url.href.split("?")[0] + "?paymentSuccess=true"
          );
          formData.set(
            "errorUrl",
            $page.url.href.split("?")[0] + "?paymentFailure=true"
          );
          isSubmitting = true;
      ...
      >
        <p class="mb-2 text-lg" style={`color:${club?.primaryColor}`}>
          Club Details
        </p>
        <div class="space-y-2">
          <Label style={`color:${club?.primaryColor}`} for="school"
            >School</Label
          >
          <Select.Root
      ...
      {#each club.termHighlights as term, index}
        <li class="mt-4 flex gap-4 max-md:text-sm w-full">
          <div
            ...
            style={`background-color:${club.secondaryColor}`}
          >
            <p class="text-sm font-bold" style={`color:${club.primaryColor}`}>
              0{index + 1}
            </p>
          </div>
  ...
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description
        >Your payment has been successful</AlertDialog.Description
      >
    </AlertDialog.Header>
    <AlertDialog.Footer>
...
<AlertDialog.Root open={failureAlertOpen == "true" ? true : false}>
  <AlertDialog.Content>
    <AlertDialog.Header>
      ...
      <AlertDialog.Description
        >Your payment has been unsuccessful</AlertDialog.Description
      >
    </AlertDialog.Header>
    <AlertDialog.Footer>
      <AlertDialog.Cancel>Close</AlertDialog.Cancel>
